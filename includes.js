...
(function () {
  ...
  /* タップで一滴、スライドで筋状に広がる白い煙。ときどき自然にも立ちのぼる。
     色は白のみ（宇宙の白い煙に統一） */
  function startInk(canvas) {
    ...
    function frame(now) {
      ...
      for (var i = parts.length - 1; i >= 0; i--) {
        ...
        var col = document.documentElement.classList.contains('light') ? '90,100,115' : '255,255,255';
        var g = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, p.r);
        g.addColorStop(0, 'rgba(' + col + ',' + alpha.toFixed(3) + ')');
        g.addColorStop(.7, 'rgba(' + col + ',' + (alpha * .45).toFixed(3) + ')');
